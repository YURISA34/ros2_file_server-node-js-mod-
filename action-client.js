        ...
        try {
            ...
            if (!result.success) {
                if (result.canceled) {
                    ...
                    node.send([{ state: 5, payload: result.result }, null ]);
                } else if (result.aborted) {
                    node.status({ fill: "red", shape: "dot", text: "goal was aborted"});
                    node.send([{ state: 6, payload: result.result }, null ]);
                } else {
                    node.status({ fill: "red", shape: "dot", text: "goal failed"});
                    node.send([{ state: 0, payload: "" }, null ]);
                }
                return;
            }
            ...
            node.send([{ state: 4, payload: result.result }, null ]);
            
        } catch (error) {
            console.log("Error in performing_action:");
            console.log(error);
            node.status({ fill: "red", shape: "dot", text: "error"});
            // node.send([{ state: 0, payload: error }, null ]);
        }
    }
